module TimeHelper
  ...
  def months(order_type=nil, start_date=nil, end_date=nil)
    ...
    if order_type
      start_date = Order.where(order_type: order_type, sale_date: start_date..end_date).order("sale_date").first.sale_date
      end_date = Order.where(order_type: order_type, sale_date: start_date..end_date).order("sale_date").last.sale_date
    else
